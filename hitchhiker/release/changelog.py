...
def _commit_cmp(a, b):
    ...
    if a.is_conventional and b.is_conventional:
        if a.get_version_bump() < b.get_version_bump():
            ...
            return 1
        elif a.get_version_bump() == b.get_version_bump() and a.get_version_bump() == enums.VersionBump.NONE:
...
def gen_changelog(change_commits, new_version, repo_owner=None, repo_name=None):
    ...
    for project in change_commits.keys():
        ...
        commits_types = {}
        commits = [(conventional.ConventionalCommitParser(commit.message), commit) for commit in change_commits[project][1]]
        ...
        for commit, gitcommit in commits:
            ...
            type = "breaking" if commit.breaking is not None and commit.breaking else type

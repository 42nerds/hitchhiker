...
def create_context_from_raw_config(tomlcfg: str, repo: git.Repo):
    ...
    if "tool.hitchhiker.projects" in tomlconf:
        for project in tomlconf["tool.hitchhiker.projects"]:
            ...
            project_ctx = {"name": project, "path": conf["path"], "version": semver.Version(), "prerelease": (conf["prerelease"] if "prerelease" in conf else False), "version_variables": [], "version_toml": [], "version_odoo_manifest": []}
